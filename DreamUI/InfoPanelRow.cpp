...
namespace InfoPanel {
	...
	Row::Row(UISimpleFrame* parent, float inHeight, float inItemWidth, unsigned int inItemCount) :
		itemCount(inItemCount), height(inHeight), itemWidth(inItemWidth), hidden(false), width(0), playerColorBg(NULL),playerRaceIcon(NULL)
	{
		float raceIconSize = RACEICON_FACTOR * std::min(height, 0.02f);
		...
		playerIdLabel = new Label(frame, "1", std::min(height, 0.02f) * 0.4f, Color(1.f, 1.f, 1.f));
	...
	void Row::update()//MUSTDO Æ¥Åä¿í¶È
	{
			...
			iter != contentText.end(); ++iter)
		{
			...
			sfs->setRelativePosition(
				...
				frame,
				POS_L,
				RACEICON_FACTOR * std::min(height, 0.02f) + 0.005f
