	...
	bShowRowFrame(true), bInverse(false), tagHeight()
{
	...
	setRowColumnCount(_rowCount, _columnCount);
...
void Table::setRowColumnCount(unsigned int _rowCount, unsigned int _columnCount)
{
	if (this->rowCount == _rowCount && this->columnCount == _columnCount)
	{
		return;
	}

	if (_rowCount < this->rowCount) //减少行数
	{
		for (unsigned int i = _rowCount; i < std::min(this->rowCount, elements.size()); ++i) //删除多余行的所有元素
		{
			for (unsigned int j = 0; j < elements[i].size(); ++j)
			{
				if (elements[i][j]) delete elements[i][j];
	...
	{
		for (unsigned int i = 0; i < std::min(_rowCount, elements.size()); ++i) //删除剩余行的多余列元素
		{
			for (unsigned int j = _columnCount; j < std::min(this->columnCount, elements[i].size()); ++j)
			{
				if (elements[i][j]) delete elements[i][j];
			}
		}
	...
	elements.resize(_rowCount);
	elementsPassiveFlag.resize(_rowCount);
	rowFrames.resize(_rowCount);
	titleElements.resize(_columnCount);
	columnMinWidths.resize(_columnCount);
	for (unsigned int i = 0; i < _rowCount; i++)
	{
		elements[i].resize(_columnCount);
		elementsPassiveFlag[i].resize(_columnCount);
	}
	bWantRefresh = true;
}
...
void Table::refresh()
{
	...
	for (unsigned int i = 0; i < rowCount; i++) //第i行
	{
		for (unsigned int j = 0; j < columnCount; j++) //第j列
		{
			...
			if (elem)
			{
				rowHeight = std::max(RowMinHeight, std::max(rowHeight,
					elementsPassiveFlag[i][j] ? 0 : elem->height()
				));
				columnWidths[j] = std::max(0.005f, std::max(columnWidths[j], elem->width()));
			}
			if (titleElements[j])
			{
				columnWidths[j] = std::max(columnWidths[j], titleElements[j]->width());
			}
			if (columnMinWidths[j])
			{
				columnWidths[j] = std::max(columnWidths[j], columnMinWidths[j]);
			}
		}
	}
	...
	for (unsigned int i = 0; i < rowCount; i++)
	{
		...
		if (bHasTag)
		{
			...
			tags[i].button->setWidth(
				std::max(totalWidth, (tags[i].button->getCaption()->width() + 0.009f))
				+ 0.002f);
