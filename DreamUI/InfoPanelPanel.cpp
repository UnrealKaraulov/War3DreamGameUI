...
namespace InfoPanel {
	Panel::Panel(
		...
		float inRowHeight)
		: rowHeight(inRowHeight), titleName(NULL), itemCount(0), itemWidth(0.01f), titleHeight(0.01f)
	{
	...
	{
		float raceIconSize = RACEICON_FACTOR * std::min(inRowHeight, 0.02f);
	...
	void Panel::updateColumn(unsigned int index) {
		if (titleIcons.count(index))
		{
			...
			st->setRelativePosition(
				...
				titleFrame,
				POS_L,
				RACEICON_FACTOR * std::min(rowHeight, 0.02f) + 0.005f
	...
	void Panel::setColumnWidth(unsigned int index, float width)
	{
		...
		titleFrame->setWidth(w + 0.01f + RACEICON_FACTOR * std::min(rowHeight, 0.02f) + 0.0035f);
