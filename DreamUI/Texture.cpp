...
Texture::Texture(
	...
	const char* _disabledTexturePath)
	: Frame(parent, width, height), colorFill(), bUseFillColor(false)
{
	...
	setTexture(_texturePath, _disabledTexturePath);
	//show(true);
}

Texture::Texture(UISimpleFrame* parent, float width, float height, const char* _texturePath /*= NULL*/, const char* _disabledTexturePath /*= NULL */)
