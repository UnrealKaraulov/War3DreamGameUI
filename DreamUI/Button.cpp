...
void Button_EventHandler(Observer* ob, unsigned int evtId) {
	...
	if (!button)
	{
		OutputScreen(10, "|cffff0000Error: Button_EventHandler(0x%X, 0x%X)|r", ob, evtId);
		return;
	}
	//TODO 需要按钮按下事件
	switch (evtId) {
...
_caption(),
_tooltip(),
controlCoverNormal(), controlCoverPushed(), controlCoverDisabled(), eventObserver(NULL), _fontHeight(4.0f)
{
	_isDisabled = false;
	_isTextShown = true;
}
...
controlCoverNormal(), controlCoverPushed(), controlCoverDisabled(), eventObserver(NULL)
{
	this->_isDisabled = false;
	this->_isTextShown = true;
	...
	sprintf_s(ButtonDefaultNameStr, 0x50, "|cffffcc00B|rutton %u", button->index);
	_caption = new Label(
		button->_control,
